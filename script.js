...
function initEventListeners() {
    ...
    // GIF拍攝相關事件
    document.getElementById('gifCameraBtn').addEventListener('click', openGifCaptureModal);
...
function displayMultipleImagePreviews(imagesArray) {
    ...
    imagesArray.forEach((imageData, index) => {
        ...
        // 檢查是否為GIF數據
        const isGif = isGifData(imageData);
        ...
        if (isGif) {
            ...
            const gifData = parseGifData(imageData);
            gifThumbnail.src = gifData.frames[0]; // 使用第一幀作為縮圖
            gifThumbnail.alt = `GIF ${index + 1}`;
            gifThumbnail.className = 'gif-thumbnail';
            
            // 添加GIF標識
            const gifLabel = document.createElement('div');
            gifLabel.className = 'gif-label';
            gifLabel.innerHTML = '🎬 GIF';
            
            // 添加播放按鈕
            const playBtn = document.createElement('div');
            playBtn.className = 'gif-play-btn';
            playBtn.innerHTML = '▶';
            playBtn.title = '點擊播放GIF';
            
            gifContainer.appendChild(gifThumbnail);
            gifContainer.appendChild(gifLabel);
            gifContainer.appendChild(playBtn);
            
            imagePreview.appendChild(gifContainer);
            
            // GIF點擊播放功能
            imagePreview.onclick = () => {
                playGifInModal(gifData, index);
            };
        } else {
            // 普通圖片預覽
        ...
        removeBtn.innerHTML = '×';
        removeBtn.title = isGif ? '刪除GIF' : '刪除圖片';
        removeBtn.onclick = (e) => {
            e.stopPropagation();
...
let gifRecordingStartTime = null;

// 開啟GIF拍攝模態視窗
function openGifCaptureModal() {
    const modal = document.getElementById('gifCaptureModal');
    const video = document.getElementById('gifCameraVideo');
    const statusElement = document.getElementById('gifStatus');
    const timerElement = document.getElementById('gifTimer');
    ...
    // 初始化攝影機
    initGifCamera().then(() => {
        statusElement.textContent = '準備拍攝';
        setupGifCaptureEvents();
    }).catch(error => {
        console.error('無法啟動攝影機:', error);
        statusElement.textContent = '攝影機啟動失敗';
        showNotification('無法啟動攝影機，請檢查權限設定', 'error');
    });
}

// 初始化GIF攝影機
async function initGifCamera() {
    try {
        const constraints = {
            video: {
                width: { ideal: 640 },
        ...
        gifCaptureStream = await navigator.mediaDevices.getUserMedia(constraints);
        const video = document.getElementById('gifCameraVideo');
        video.srcObject = gifCaptureStream;
...
// 設置GIF拍攝事件監聽器
function setupGifCaptureEvents() {
    // 關閉按鈕
    document.querySelector('.gif-capture-close').onclick = closeGifCaptureModal;
    
    // 控制按鈕
    document.getElementById('startGifBtn').onclick = startGifRecording;
    document.getElementById('stopGifBtn').onclick = stopGifRecording;
    document.getElementById('retakeGifBtn').onclick = retakeGif;
    document.getElementById('saveGifBtn').onclick = saveGifToMarker;
    
    // 時長滑桿
    const durationSlider = document.getElementById('gifDuration');
    const durationValue = document.getElementById('gifDurationValue');
...
function startGifRecording() {
    const video = document.getElementById('gifCameraVideo');
    const canvas = document.getElementById('gifCaptureCanvas');
    const ctx = canvas.getContext('2d');
    const statusElement = document.getElementById('gifStatus');
    const timerElement = document.getElementById('gifTimer');
    const duration = parseFloat(document.getElementById('gifDuration').value) * 1000; // 轉換為毫秒
    ...
    // 重置幀數組
    gifFrames = [];
    gifRecordingStartTime = Date.now();
    
    // 更新UI
    statusElement.textContent = '錄製中...';
    document.getElementById('startGifBtn').style.display = 'none';
    document.getElementById('stopGifBtn').style.display = 'inline-block';
    
    // 開始捕獲幀
    const frameRate = 10; // 每秒10幀
    const frameInterval = 1000 / frameRate;
    let lastFrameTime = 0;
    
    function captureFrame() {
        const currentTime = Date.now();
        const elapsed = currentTime - gifRecordingStartTime;
        ...
        gifRecordingTimer = requestAnimationFrame(captureFrame);
    }
...
function stopGifRecording() {
    if (gifRecordingTimer) {
        cancelAnimationFrame(gifRecordingTimer);
    ...
    document.getElementById('retakeGifBtn').style.display = 'inline-block';
    document.getElementById('saveGifBtn').style.display = 'inline-block';
    
    // 生成GIF
    generateGifFromFrames().then(gifDataUrl => {
        // 顯示預覽
        const previewContainer = document.getElementById('gifPreviewContainer');
        const previewImg = document.getElementById('gifPreviewImg');
        const video = document.getElementById('gifCameraVideo');
        
        previewImg.src = gifDataUrl;
        previewContainer.style.display = 'block';
        video.style.display = 'none';
        
        statusElement.textContent = '錄製完成';
        
        // 存儲GIF數據
        previewContainer.dataset.gifData = gifDataUrl;
    }).catch(error => {
        console.error('GIF生成失敗:', error);
        statusElement.textContent = 'GIF生成失敗';
        showNotification('GIF生成失敗，請重試', 'error');
    });
}
...
function retakeGif() {
    ...
    document.getElementById('startGifBtn').style.display = 'inline-block';
    document.getElementById('stopGifBtn').style.display = 'none';
    document.getElementById('retakeGifBtn').style.display = 'none';
    document.getElementById('saveGifBtn').style.display = 'none';
}

// 儲存GIF到標註點
function saveGifToMarker() {
    const previewContainer = document.getElementById('gifPreviewContainer');
    const gifData = previewContainer.dataset.gifData;
    
    if (!gifData) {
        showNotification('沒有可儲存的GIF', 'error');
        return;
    }
    
    // 壓縮GIF（如果需要）
    compressGif(gifData).then(compressedGifData => {
        // 添加到現有圖片數據中
        const form = document.getElementById('markerForm');
        const existingImages = JSON.parse(form.dataset.imageData || '[]');
        ...
        if (existingImages.length >= 6) {
            showNotification('已達到最大檔案數量限制(6個)', 'warning');
            return;
        }
        
        // 添加GIF數據
        existingImages.push(compressedGifData);
        
        // 更新預覽
        displayMultipleImagePreviews(existingImages);
        
        // 關閉GIF拍攝模態視窗
        closeGifCaptureModal();
        
        showNotification('GIF已成功添加到標註點', 'success');
    }).catch(error => {
        console.error('GIF壓縮失敗:', error);
        showNotification('GIF處理失敗', 'error');
    });
}
...
async function generateGifFromFrames() {
    if (gifFrames.length === 0) {
        throw new Error('沒有可用的幀數據');
    }
    
    // 簡化版本：使用第一幀作為靜態圖片（實際應用中可以使用GIF.js庫）
    // 這裡我們創建一個包含所有幀的數據結構，標記為GIF
    const gifData = {
        type: 'gif',
        frames: gifFrames,
        duration: parseFloat(document.getElementById('gifDuration').value),
        frameRate: 10
    };
    
    // 將GIF數據編碼為base64字符串
    const gifDataString = JSON.stringify(gifData);
    const gifDataUrl = 'data:application/json;base64,' + btoa(gifDataString);
    
    return gifDataUrl;
}

// 壓縮GIF
async function compressGif(gifDataUrl, maxSizeKB = 100) {
    try {
        // 對於GIF數據，我們可以減少幀數或降低質量
        const gifData = JSON.parse(atob(gifDataUrl.split(',')[1]));
        
        // 減少幀數以壓縮大小
        const compressionRatio = Math.max(0.5, maxSizeKB / 200); // 根據目標大小調整
        const targetFrameCount = Math.max(5, Math.floor(gifData.frames.length * compressionRatio));
        
        if (gifData.frames.length > targetFrameCount) {
            const step = gifData.frames.length / targetFrameCount;
            const compressedFrames = [];
            
            for (let i = 0; i < targetFrameCount; i++) {
                const frameIndex = Math.floor(i * step);
                compressedFrames.push(gifData.frames[frameIndex]);
            }
            
            gifData.frames = compressedFrames;
        }
        
        // 重新編碼
        const compressedDataString = JSON.stringify(gifData);
        const compressedDataUrl = 'data:application/json;base64,' + btoa(compressedDataString);
        
        return compressedDataUrl;
    } catch (error) {
        console.error('GIF壓縮失敗:', error);
        return gifDataUrl; // 返回原始數據
    }
}

// 重置GIF拍攝狀態
function resetGifCaptureState() {
    if (gifRecordingTimer) {
        cancelAnimationFrame(gifRecordingTimer);
        gifRecordingTimer = null;
    }
    ...
    gifFrames = [];
    gifRecordingStartTime = null;
}

// 關閉GIF拍攝模態視窗
function closeGifCaptureModal() {
    const modal = document.getElementById('gifCaptureModal');
    
    // 停止攝影機
    if (gifCaptureStream) {
        gifCaptureStream.getTracks().forEach(track => track.stop());
...
// 檢查數據是否為GIF格式
function isGifData(dataUrl) {
    return dataUrl && dataUrl.startsWith('data:application/json;base64,');
}

// 解析GIF數據
function parseGifData(dataUrl) {
    try {
        if (!isGifData(dataUrl)) {
            return null;
        }
        
        const jsonData = atob(dataUrl.split(',')[1]);
        const gifData = JSON.parse(jsonData);
        
        if (gifData.type === 'gif' && gifData.frames && gifData.frames.length > 0) {
            return gifData;
        }
        
        return null;
    } catch (error) {
        console.error('GIF數據解析失敗:', error);
        return null;
    }
}

// 在模態視窗中播放GIF
function playGifInModal(gifData, index) {
    const modal = document.getElementById('imagePreviewModal');
    const modalImg = document.getElementById('modalPreviewImg');
    const prevBtn = document.getElementById('prevImageBtn');
    const nextBtn = document.getElementById('nextImageBtn');
    ...
    // 開始播放GIF動畫
    playGifAnimation(modalImg, gifData);
...
function playGifAnimation(imgElement, gifData) {
    ...
    const frameDelay = (gifData.duration * 1000) / frames.length; // 每幀延遲時間
    currentGifFrame = 0;
    
    function showNextFrame() {
        if (currentGifFrame >= frames.length) {
            currentGifFrame = 0; // 循環播放
        }
        
        imgElement.src = frames[currentGifFrame];
        currentGifFrame++;
        
        gifAnimationTimer = setTimeout(showNextFrame, frameDelay);
    }
